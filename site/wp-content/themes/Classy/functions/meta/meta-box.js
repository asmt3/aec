jQuery(document).ready(function($) {
	...
	// colorpicker field
	$('.classy-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.classy-color-select').click(function(){
		$parent = $(this).parent();
		$(this).siblings('.classy-color-picker').toggle();
		$('.classy-color-select', $parent).toggle();
		return false;
	});

	// add more file
	$('.classy-add-file').click(function(){
	...
	// delete file
	$('.classy-upload').delegate('.classy-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
	...
	$('.classy-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.classy-images-data').val();
	...
	// thickbox upload
	$('.classy-upload-button').click(function(){
	...
	// thickbox upload
    window.formfield = '';
    $('.classy-upload-single-image').live('click', function() {
        window.formfield = $('.classy-single-image',$(this).parent());
        tb_show('', 'media-upload.php?type=image&TB_iframe=true');
        return false;
    });
    window.classy_original_send_to_editor = window.send_to_editor;
    window.send_to_editor = function(html) {
        if (window.formfield) {
            ...
            window.classy_original_send_to_editor(html);
        }
        window.formfield = '';
